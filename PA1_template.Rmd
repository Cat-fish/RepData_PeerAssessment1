...
1. total steps per day and interval
```{r}
...
```{r}
head(order(tspdi_avr,decreasing=T))
```
...
hist2 <- hist(tspd_fill$tspd,breaks=50,col=rgb(0,0,1,1/4),xlab="number of steps taken per day (NA filled)", main="Total number of steps taken each day")

hist1 <- hist(tspd$tspd,breaks=50, col=rgb(1,0,0,1/4), xlab="number of steps taken per day (Original)", main="Total number of steps taken each day")  
```
...
legend("topright", legend = c("Original data", "Filled data"),
...
Since the compliment was done by filling in the average value, the mean and median values are almsot identical between the original and the filled data.
